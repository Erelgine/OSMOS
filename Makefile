...
setup: workspace.check workspace.setup build.clean build.image.unmount build.image build.all build.image.unmount
...
workspace.check:
	...
	if [[ -e /usr/sbin/grub-install && -e /usr/bin/grub-file ]]; then \
		echo -e "found"; \
		if [ "$(GRUB_NAME)" != "grub" ]; then \
			echo -e "Error: Please modify the main Makefile variable \"GRUB_NAME\"\nwith the value \"grub\"."; \
			exit 1; \
		fi; \
    elif [[ -e /usr/sbin/grub2-install && -e /usr/bin/grub2-file ]]; then \
        echo -e "found"; \
	else \
		echo -e "not found\nYou can find and install the specific package by using the distribution's package manager (usually apt(itude), pacman, or yum, with administrator privileges)"; \
		exit 1; \
...
build.all-force: workspace.check workspace.check-setup build.image.unmount build.clean build.image build.system
build.all: workspace.check workspace.check-setup build.image.mount build.system build.image.unmount
...
build.image: build.image.unmount build.image.generate build.image.mount-image build.image.mkfs build.image.mount-fs build.image.grub
...
build.image.mount-fs:
	...
	if [ ! -d $(FOLDER_VDRIVE) ]; then sudo mkdir -p $(FOLDER_VDRIVE); fi;
	$(SUDO) mount | grep -qs "$(FOLDER_VLOOP)p1"; \
	if [ "$$?" == "0" ]; then \
		echo -e "already mounted"; \
	else \
...
build.image.unmount-image:
	...
	$(SUDO) losetup | grep -qs "$(FOLDER_VLOOP)" 1>/dev/null; \
	if [ "$$?" == "1" ]; then \
		echo -e "already unmounted"; \
	else \
		$(SUDO) losetup -d $(FOLDER_VLOOP) 1>/dev/null 2>/dev/null; \
		echo -e "unmount done"; \
	fi;
...
build.image.grub:
	...
	sudo mkdir -p $(FOLDER_VDRIVE)/boot/$(GRUB_NAME);
	...
	echo -en "(hd0) /dev/sda" | sudo tee --append $(FOLDER_VDRIVE)/boot/$(GRUB_NAME)/device.map >> /dev/null
	echo -en "mkmap ";
	# We install GRUB
	sudo $(GRUB_NAME)-install --no-floppy --grub-mkdevicemap=$(FOLDER_VDRIVE)/boot/$(GRUB_NAME)/device.map --root-directory=$(FOLDER_VDRIVE)/ $(FOLDER_BINARY)/hdd.img>>/dev/null 1>>/dev/null 2>>/dev/null
	# We add the necessary entry in order to boot into OSMOS (with the multiboot2 specification)
	echo -en "mkgrub ";
	echo -en "menuentry 'OSMOS kernel' {\necho 'Starting OSMOS...'\nset root='hd0,1'\nmultiboot2 /boot/core/boot.bin\n}" | sudo tee --append $(FOLDER_VDRIVE)/boot/$(GRUB_NAME)/grub.cfg >> /dev/null
	# and finished !
